...
socio_economic_task = Task(
    description=(
        ...
        "reports on socio-economic conditions in conflict-affected regions. Delegate all collected data to the Evaluator Agent for validation."
    ),
    expected_output=(
...
evaluator_agent = Agent(
    ...
    goal="Verify and validate all data collected by the specialized agents (Political, Military, Socio-Economic, Global) "
         "before allowing it to be compiled into summaries. Ensure the accuracy and credibility of all information before it is used.",
    backstory="The Evaluator Agent is an expert in data validation and research methodologies, ensuring the integrity of information "
              "gathered by the other agents. The agent uses analytical skills to cross-check data against reliable sources, rejecting "
              "any unverified or questionable data.",
    verbose=True,
    allow_delegation=False,
    tools=[tavily_search_tool]  # Placeholder, fill in tools here
)

evaluator_task = Task(
    description=(
        "Review and validate the data collected by the Political, Military, Socio-Economic, and Global Perspective Agents. "
        "Ensure that all information is accurate, relevant, and supported by credible references. Cross-check data with external "
        "trusted sources, and approve or reject data submissions based on validation criteria."
    ),
    expected_output=(
        "A validated set of summaries from all agents, containing verified and trustworthy information. The summaries will include: \n"
        "1. Verified political, military, socio-economic, and global summaries \n"
        "2. References to trusted sources that validate the data \n"
        "3. Approval for the Narrative Generator to proceed with the final report creation"
    ),
...
narrative_generator_agent = Agent(
    ...
    goal="Synthesize the summaries from the specialized agents into a cohesive and comprehensive report. "
         "Present a clear and insightful narrative on the current state of the Communist Party of the Philippines (CPP) "
         "based solely on the summaries provided by the Political, Military, Socio-Economic, and Global agents.",
    ...
    tools=[tavily_search_tool]  # No external tools required
)

narrative_task = Task(
    description=(
        "Synthesize the summaries provided by the Political, Military, Socio-Economic, and Global agents into a single comprehensive report. "
...
manager = Agent(
    ...
    backstory="The Manager Agent holds advanced academic credentials and has a deep understanding of the Philippines' complex political, "
              ...
              "The Manager Agent uses this background to ask targeted, research-based questions of the specialized agents and ensures their outputs provide a comprehensive, "
              "accurate assessment of the CPP’s current state.",
    verbose=True,
...
crew = Crew(
    agents=[political_agent, military_agent, socio_economic_agent, global_perspective_agent, evaluator_agent, narrative_generator_agent],
